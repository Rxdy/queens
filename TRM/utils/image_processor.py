...
def find_closest_zone_color(rgb_color):
    ...
    if min_distance > DISTANCE_THRESHOLD:
        logger.debug(f"Couleur indéterminée RGB{rgb_color}: distance min={min_distance:.1f} > seuil={DISTANCE_THRESHOLD}")
        return EMPTY_VALUE
...
def extract_matrix_from_image(image_data: bytes, expected_size: int = None):
    ...
    try:
        ...
        logger.info(f"Zones distinctes: {distinct_zones}, cellules vides: {empty_count}, attendu: {grid_size} zones")
        ...
        if wrong_zone_count or too_many_empty:
            logger.warning(f"Validation échouée ({distinct_zones} zones distinctes, {empty_count} vides). Clustering K-means++...")
...
def refine_zones_with_clustering(image_array, grid_size):
    ...
    if len(cluster_centers) < num_clusters:
        remaining = [f for f in color_features if not any(np.array_equal(f, c) for c in cluster_centers)]
        for _ in range(num_clusters - len(cluster_centers)):
            if not remaining:
                break
            dists = [min(np.linalg.norm(feature - center) for center in cluster_centers) for feature in remaining]
    ...
    if len(used) != num_clusters:
        ...
        for _ in range(1, num_clusters):
            dists = [min(np.linalg.norm(feature - center) for center in cluster_centers) for feature in color_features]
        ...
        for _ in range(20):
            ...
            for cid in range(num_clusters):
                cluster_feats = [np.array(cell_colors[i], dtype=float) for i in valid_indices if assignments[i] == cid]
                if cluster_feats:
    ...
    logger.info(f"K-means++ terminé: {num_clusters} clusters, {len(set(a for a in assignments if a != -1))} distincts")
    
    # Remapper les clusters vers les indices de la palette frontend (assignment unique)
    cluster_centers = []
    for cluster_id in range(num_clusters):
        cluster_colors = [np.array(cell_colors[i], dtype=float) for i in valid_indices if assignments[i] == cluster_id]
        if cluster_colors:
